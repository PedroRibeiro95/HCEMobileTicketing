...
{	
	...
	*((db_int*)(mmp->next_front)) = size;	/* Set the size. */
	...
	ptr = POINTERATNBYTES(mmp->next_front, sizeof(db_int), void*);
	//ptr = mmp->next_front + sizeof(db_int);
...
void db_qmm_ffree(db_query_mm_t *mmp, void *ptr)
{
...
/* Free memory off the front stack. */
void db_qmm_bfree(db_query_mm_t *mmp, void *ptr)
{
	/* Retrieve the size of the memory chunk we are to free. If it is
	   flagged, remove the flag. */
	//db_int size = *POINTERATNBYTES(ptr, -1*sizeof(db_int), db_int*);
	db_int size, test_val;
	memcpy(&size, (unsigned char*) ptr + (-1*sizeof(db_int)), sizeof(db_int));
	if ((size & (1 << ((8*sizeof(db_int))-1))) ==
		(1 << ((8*sizeof(db_int))-1)))
	{
	...
	else if (POINTERATNBYTES(ptr, -1*sizeof(db_int), void*) == mmp->last_back)
	{
		...
		do
		{
			...
			memcpy(&size, (char*) mmp->last_back, sizeof(db_int));
			if ((size & (1 << ((8*sizeof(db_int))-1))) ==
				(1 << ((8*sizeof(db_int))-1)))
			{
				size &= ~(1 << ((8*sizeof(db_int))-1));
...
{	
	db_int dist, tempsize;
	//void **temp_addr = malloc(sizeof(void**));
	/* Check that size requested is valid. */
	if (size > mmp->size)
	{
		mmp->errcode = -2;
	...
	IMSG("db_query_mm.c db_qmm_fextend after if\n");
	...
	IMSG("db_query_mm.c db_qmm_fextend after second if\n");
	...
	IMSG("db_query_mm.c db_qmm_fextend after first memcpy\n");
	...
	//temp_addr = (void**) ((unsigned char*) mmp->next_front) + (-1*sizeof(void*));
	//memcpy(temp_addr, ((unsigned char*) mmp->next_front) + (-1*sizeof(void*)), 8);
	//*temp_addr = POINTERATNBYTES(mmp->next_front, -1*(tempsize+sizeof(db_int)+sizeof(void*)), void*);

	//*(((unsigned char *) mmp->next_front) + (-1*sizeof(void*))) = 
	//	POINTERATNBYTES(mmp->next_front, -1*(tempsize+sizeof(db_int)+sizeof(void*)), void*);

	printf("tempsize %d next_front %s\n", tempsize, (char*) mmp->next_front);

	memcpy((((unsigned char *) mmp->next_front) + (-1*sizeof(void*))), (((unsigned char *) mmp->next_front) + (-1*(tempsize+sizeof(db_int)+sizeof(void*)))), 8);
...
{	
	...
	else if (((unsigned char*) mmp->last_back) - ((unsigned char*) mmp->segment) < size)
	{
		mmp->errcode = -1;
		return NULL;
	}
	...
	tempsize = *((db_int*)(mmp->last_back)) + size;
