...
{
	signed int size = queryTreeToStringSize(root, 0);
	if (size != -1)
	{
...
db_int whichScan(db_int offset, db_lexer_t *lexerp, scan_t *tables, db_uint8 numtables)
{
	...
	for (i = 0; i < (db_int)numtables; ++i)
	{
		...
		tablename = malloc(sizeof(char) * (gettokenlength(&token)+1));
	...
	*numtablesp = 0;
	...
	while (end > lexerp->offset && 1==lexer_next(lexerp))
	{
		...
		if ((db_uint8)DB_LEXER_TT_IDENT == lexerp->token.type)
		{
			...
			tempscanp = &((*tablesp)[(int)(*numtablesp)]);
		...
		scan_t *tables, db_uint8 numtables)
{
	...
	eetarr = db_qmm_falloc(mmp, ((int)numexpressions)*sizeof(db_eet_t));
	if (NULL == eetarr)
	{
		DB_ERROR_MESSAGE("out of memory",
			lexerp->offset, lexerp->command);
		...
		if ((!(end > lexerp->offset && 1==lexer_next(lexerp)) || DB_LEXER_TT_COMMA == lexerp->token.type))
		{
			...
			else if (0 != last.end - last.start && DB_LEXER_TT_IDENT == last.type)
			{
				...
				if (0 != secondlast.end - secondlast.start && DB_LEXER_TT_OP != secondlast.type && DB_LEXER_TT_IDENTCONJ != secondlast.type)
				{
					...
					aliasname = db_qmm_balloc(mmp, tokensize+1);	/* + 1 for null byte. */
...
db_op_base_t* parse(char* command, db_query_mm_t* mmp)
{
	...
	db_int retval;
	db_int8 whichindex;
	db_eetnode_attr_t *hold_attr;
	db_eetnode_dbint_t *hold_int;
	db_eet_t *hold_eet;
	db_tuple_t *hold_tuple;
			...
			else
			{
				...
				if (1 == numtables && NULL != eetp && NULL != eetp->nodes)
				{
					...
					if (1==valid && NULL != val && NULL != relop && NULL != attr)
					{
						...
						memcpy(hold_attr, attr, sizeof(db_eetnode_attr_t));
						whichindex = findindexon(tables, hold_attr);
						//whichindex = findindexon(tables, attr);
						if (-1 != whichindex)
						{
							/* Setup pre-condition. */
							...
							if (DB_EETNODE_OP_GTE == relop->type || DB_EETNODE_OP_EQ == relop->type)
							{
								...
								memcpy(hold_int, val, sizeof(db_eetnode_dbint_t));
								hold_eet = malloc(sizeof(db_eet_t));
								memcpy(hold_eet, &hold_attr->pos, sizeof(db_eet_t));
								hold_tuple = malloc(sizeof(db_tuple_t));
								memcpy(hold_tuple, &hold_int->integer, sizeof(db_tuple_t));
								...
								tables->tuple_start = db_index_getoffset(tables, whichindex, hold_eet, hold_tuple, NULL, mmp);
							...
							else if (DB_EETNODE_OP_GT == relop->type)
							{
								...
								tables->tuple_start = db_index_getoffset(tables,whichindex, hold_eet, hold_tuple+1, NULL, mmp);
								rewind_scan(tables, mmp);
								free(hold_int);
								free(hold_eet);
								free(hold_tuple);
							...
							if (DB_EETNODE_OP_LTE == relop->type || DB_EETNODE_OP_EQ == relop->type)
							{
								...
								tables->stopat = hold_int->integer + 1;
							...
							else if (DB_EETNODE_OP_LT == relop->type)
							{
								...
								tables->stopat = hold_int->integer;
								tables->indexon = whichindex;
								free(hold_int);
							}
						}
