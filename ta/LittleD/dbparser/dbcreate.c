...
db_int createTable(db_lexer_t *lexerp, db_int end, db_query_mm_t *mmp)
{
	...
	if (1!=lexer_next(lexerp) || DB_LEXER_TT_LPAREN != lexerp->token.type)
	{
		...
		db_fileremove(tablename, newtable);
		db_qmm_ffree(mmp, tablename);
		return -1;
	}
	
	attrcount = 0;
	arr = db_qmm_balloc(mmp, 0);
	...
	while (1==lexer_next(lexerp) && DB_LEXER_TT_RPAREN != lexerp->token.type)
	{
		...
		{
			length = gettokenlength(&(lexerp->token));
			attrname = malloc(sizeof(char) * (length+1));
	...
	while (attrcount > 0)
	{
		...
		length = (db_uint8)gettokenlength(&(arr[((int)attrcount)-1]))+1;
		attrname = malloc(sizeof(char) * ((int)length));
	...
	{
		DB_ERROR_MESSAGE("missing ')'", lexerp->offset, lexerp->command);
		db_fileremove(tablename, newtable);  //FIXME
		db_qmm_ffree(mmp, tablename);
		return -1;
	}
