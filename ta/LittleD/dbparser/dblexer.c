...
@param		lexerp		A pointer to the lexer variable that
				generated the token.
*/
void printtoken(db_lexer_token_t *tokenp, db_lexer_t *lexerp) //MAY NOT BE NEEDED
{
...
db_int isstartofoperator(db_lexer_token_t *tokenp, db_lexer_t *lexerp)
{
	...
	for (; i < sizeof(operators) / sizeof(struct keyword); ++i)
	{
		...
		for (; j < tokenp->end - tokenp->start && j < (int) sizeof(operators[i]); ++j)
		{
	...
	while (lexible(lexerp) && !done)
	{
		switch(mode)
		{
			...
			case DB_LEXER_MODE_OP:
				...
				{
					/* Check for comments. */
					if (token_stringequal(&newtoken, (char *) "--", 2, lexerp, 0))
					{
