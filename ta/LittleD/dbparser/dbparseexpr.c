...
db_int parseexpr_pop(db_int type, db_eetnode_t **exprp, db_eetnode_t *stack, db_eetnode_t **stack_topp, db_int *sizep, db_uint8 limit, db_query_mm_t *mmp)
{
	...
	while (*stack_topp != stack && limit != (*stack_topp)->type && (type == -1 || compareopprecedence(type, (*stack_topp)->type) >= 0))
	{
		...
		newnodep = POINTERATNBYTES(*exprp, *sizep, db_eetnode_t*);
...
{
	db_eetnode_t *stack;
	db_eetnode_t *stack_top;
	db_eetnode_dbstring_t *newnodepstr;
	db_eetnode_dbint_t *newnodepint;
	db_eetnode_placeholder_t *newnodepplc;
	db_eetnode_attr_t *newnodepattr;
	db_eetnode_t *newnodepeet;
	db_eetnode_aggr_temp_t *newnodepaggrt;

	int aux_int;

	db_eetnode_aggr_temp_t *hold_aggrt;

	db_int attr_s, attr_e, rel_s, rel_e, db_s, db_e;
	db_int tokenlength;
	db_int offset;
	db_uint8 counter;
	...
	while (lexerp->offset < end && 1==lexer_next(lexerp) && lexerp->token.start < end)
	{
		if ((db_uint8)DB_LEXER_TT_INT == lexerp->token.type)
		{
			...
			//newnodepint = POINTERATNBYTES(*exprp, size, db_eetnode_dbint_t*);
			newnodepint = malloc(sizeof(db_eetnode_dbint_t));
			memcpy(newnodepint, (unsigned char *) *exprp + size, sizeof(db_eetnode_dbint_t));
			size+=sizeof(db_eetnode_dbint_t);
			
			/* Build the node! */
			newnodepint->base.type = DB_EETNODE_CONST_DBINT;
			//char temp[gettokenlength(&(lexerp->token))+1];
			temp = malloc(sizeof(char) * gettokenlength(&(lexerp->token))+1);
			gettokenstring(&(lexerp->token), temp, lexerp);
			newnodepint->integer = atoi(temp);	// TODO: Better way to do this?  Maybe without using library?
		...
		else if ((db_uint8)DB_LEXER_TT_STRING == lexerp->token.type)
		{
			...
			newnodepstr = malloc(sizeof(db_eetnode_dbstring_t));
			//newnodepstr = POINTERATNBYTES(*exprp, size, db_eetnode_dbstring_t*);
			memcpy(newnodepstr, (unsigned char *) *exprp + size, sizeof(db_eetnode_dbstring_t));
			size+=sizeof(db_eetnode_dbstring_t);
			
			/* For now, simply store beginning offset of string in
			   pointer. Its a hack, but it works. */
			newnodepstr->base.type = DB_EETNODE_CONST_DBSTRING;
			aux_int = lexerp->token.start-1;
			memcpy(newnodepstr->string, &aux_int, sizeof(char)); //WARNING!!!
			//newnodepstr->string = (char*)(lexerp->token.start-1);
		...
		else if ((db_uint8)DB_LEXER_TT_PLACEHOLDER == lexerp->token.type)
		{
			...
			newnodepplc = malloc(sizeof(db_eetnode_placeholder_t));
			//newnodepplc = POINTERATNBYTES(*exprp, size, db_eetnode_placeholder_t*);
			memcpy(newnodepplc, (unsigned char *) *exprp + size, sizeof(db_eetnode_placeholder_t));
			size+=sizeof(db_eetnode_placeholder_t);
			
			/* Build the node! */
			newnodepplc->base.type = DB_EETNODE_PLACEHOLDER;
			newnodepplc->placeholdertype	= lexerp->token.info;
			newnodepplc->offset		= lexerp->token.end;
		...
		else if ((db_uint8)DB_LEXER_TT_AGGRFUNC == lexerp->token.type)
		{
			...
			hold_aggrt = malloc(sizeof(db_eetnode_aggr_temp_t));
			memcpy(hold_aggrt, stack_top, sizeof(db_eetnode_aggr_temp_t));
			hold_aggrt->aggr_type = lexerp->token.bcode;
			memcpy(stack_top, hold_aggrt, sizeof(db_eetnode_aggr_temp_t));
			free(hold_aggrt);
			...
			else if ((db_uint8)DB_EETNODE_LPAREN == stack_top->type)
			{
				if (NULL!=(stack_top=db_qmm_bextend(mmp, (db_int)(-sizeof(db_eetnode_t)))) && stack_top != stack)
				{
					if (ISFUNC(stack_top->type))
					{
						...
						newnodepeet = malloc(sizeof(db_eetnode_t));
						//newnodepeet = POINTERATNBYTES(*exprp, size, db_eetnode_t*);
						memcpy(newnodepeet, (unsigned char *) *exprp + size, sizeof(db_eetnode_t));
						size+=sizeof(db_eetnode_t);
						*newnodepeet = *stack_top;
					...
					else if ((db_uint8)DB_EETNODE_AGGR_TEMP == stack_top->type)
					{
						...
						newnodepaggrt = malloc(sizeof(db_eetnode_aggr_temp_t));
						//newnodepaggrt = POINTERATNBYTES(*exprp, size, db_eetnode_aggr_temp_t*);
						memcpy(newnodepaggrt, (unsigned char*) *exprp + size, sizeof(db_eetnode_aggr_temp_t));
						size+=sizeof(db_eetnode_aggr_temp_t);
						memcpy(newnodepaggrt, &stack_top, sizeof(db_eetnode_aggr_temp_t));
						//*newnodepaggrt = *((db_eetnode_aggr_temp_t*)stack_top);
	...
	if (jointolast)
	{
		...
		newnodepeet = POINTERATNBYTES(*exprp, size, db_eetnode_t*);
		size+=sizeof(db_eetnode_t);
		newnodepeet->type = DB_EETNODE_OP_AND;
	}
	...
	while (POINTERBYTEDIST(stack, *exprp) < size)
	{
		...
		{
			newnodepstr = malloc(sizeof(db_eetnode_dbstring_t));
			//newnodepstr = (db_eetnode_dbstring_t*)stack;
			memcpy(newnodepstr, stack, sizeof(db_eetnode_dbstring_t));
			//lexerp->offset = (db_int)(newnodepstr->string);
			memcpy(&lexerp->offset, newnodepstr->string, sizeof(db_int));
			lexer_next(lexerp);
			
			/* Get length of the token string. */
			tokenlength = gettokenlength(&(lexerp->token));
			/* Allocate memory on front for new string. */
			// TODO: BALLOC if possible?
			string = db_qmm_balloc(mmp, tokenlength + 1);
...
db_int setupattribute(db_eetnode_attr_t *attrnodep, db_lexer_t *lexerp, db_op_base_t *evalpoint, scan_t *scans, db_uint8 numscans, int depth)
{
	...
	{
		retval = setupattribute(attrnodep, lexerp, ((db_op_onechild_t*)evalpoint)->child, scans, numscans, depth+1);
		if (1 == retval)
		{
			attrnodep->tuple_pos = 0;
		}
	...
	else
	{
		...
		if (1 == attrnodep->pos)
		{
			...
			attrname = malloc(sizeof(char) * gettokenlength(&temptoken)+1);
		...
		{
			gettokenat(&temptoken, *lexerp, attrnodep->tokenstart, 0);
			tablename = malloc(sizeof(char) * gettokenlength(&temptoken)+1);
...
db_int verifysetupattributes(db_eet_t *eetp, db_lexer_t *lexerp, db_op_base_t *evalpoint, scan_t *scans, db_uint8 numscans, int startdepth)
{
	...
	while (POINTERBYTEDIST(cursor, eetp->nodes) < eetp->size)
	{
		if (DB_EETNODE_ATTR == cursor->type)
		{
			...
			retval = setupattribute(hold_attr, lexerp, evalpoint, scans, numscans, startdepth);
			if (1 != retval)
			{
				free(hold_attr);
				return retval;
