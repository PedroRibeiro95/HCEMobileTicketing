...
db_int sizeQuery(db_op_base_t *op, db_tuple_t *next_t, db_query_mm_t *mmp)
{
	...
	requiredMem = 1 + 3*sizeRow;	/* Null-byte + first row
...
char* formatQuery(db_op_base_t *op, db_query_mm_t *mmp)
{
	...
	widths = calloc((db_int) (op->header->num_attr), sizeof(db_int));
	computeWidths(op, widths);
	requiredMem = sizeQuery(op, &next_t, mmp);
	out = calloc(requiredMem, sizeof(char));
	...
	for (i = 0; i < (db_int) op->header->num_attr; ++i)
	{
		...
		tmp = calloc(widths[i] + 3, sizeof(char));
				...
				|| op->header->size_name[i] <= 0)
		{
			...
			{
				snprintf(tmp, widths[i] + 3, " %*s ", widths[i], "INTEGER");
				strcat(out, tmp);
			}
			else if ((db_uint8) DB_STRING == op->header->types[i])
			{
				snprintf(tmp, widths[i] + 3, " %*s(%d) ", widths[i]-numdbintdigits((db_int)op->header->sizes[i])-1, "STRING", (db_int)(op->header->sizes[i]) - 1);
				strcat(out, tmp);
			}
		}
		...
		{
			snprintf(tmp, widths[i] + 3, " %*s ", widths[i], op->header->names[i]);
			strcat(out, tmp);
		}
		free(tmp);
		strcat(out, "|");
...
char* formatTuple(db_tuple_t *toprint, db_op_base_t *op, db_int *widths)
{
	...
	for (i = 0; i < (db_int) op->header->num_attr; ++i)
	{
		tmp = calloc(widths[i]+3, sizeof(char));	/* 2 spaces, null-byte. */
		if (((toprint->isnull[i / 8] & (1 << (i % 8))) >> (i % 8)) == 1)
		{
			snprintf(tmp, widths[i] + 3, " %*s ", widths[i], "NULL");
		}
		else if ((db_uint8) DB_INT == op->header->types[i])
		{
			snprintf(tmp, widths[i] + 3, " %*d ", widths[i],
					getintbypos(toprint, i, op->header));
		}
		else if ((db_uint8) DB_STRING == op->header->types[i])
		{
			snprintf(tmp, widths[i] + 3, " %*s ", widths[i],
