...
{	
	...
	result = createnewheader(pp->base.header,
...
db_int next_project(project_t *pp, db_tuple_t *next_tp, db_query_mm_t *mmp)
{
	...
	result = next(pp->child, &temp_t, mmp);
	if (1 != result)
	{
					...
					&(pp->child->header), 0, mmp))
				{
					...
					case 1:
						...
						/**((db_int*)(&next_tp->bytes[pp->base.header->offsets[k]])) =
							value;*/
						memcpy(&next_tp->bytes[pp->base.header->offsets[k]], &value, sizeof(db_int));
						break;
					case -1:
					default:
						close_tuple(&temp_t, mmp);
			...
			else if (pp->base.header->types[k] == DB_STRING)
			{
					...
					&(pp->child->header), 0, mmp))
				{
					...
					case 1:
						/* Make sure this attributes isnull bit is 0. */
						next_tp->isnull[(k / 8)] &= ~(1 << (k % 8));
						for (j = 0; (unsigned int) j < strlen(value); j++)
						{
