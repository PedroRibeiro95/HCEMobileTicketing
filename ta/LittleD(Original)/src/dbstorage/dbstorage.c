...
db_int db_fileexists(char *filename)
{
	...
	res = TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, &db_id, sizeof(int),
		flags, &newtable);
	if (res != TEE_SUCCESS)
		return 0;
...
void db_openreadfile(char *filename, TEE_ObjectHandle table, int db_id)
{
	...
	TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, &db_id, sizeof(int),
		flags, &table);
#endif
}
...
void db_openwritefile(char *filename, TEE_ObjectHandle newtable, int db_id)
{
	...
	TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, db_id, sizeof(int),
